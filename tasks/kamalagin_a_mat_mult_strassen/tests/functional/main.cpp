...
class KamalaginARunFuncTestsMatMult : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 ...
 protected:
  ...
  for (int i = 0; i < n; ++i) {
    for (int k = 0; k < n; ++k) {
      const double a = input_data_.A[static_cast<std::size_t>(i) * n + k];
      for (int j = 0; j < n; ++j) {
        expected[static_cast<std::size_t>(i) * n + j] +=
            a * input_data_.B[static_cast<std::size_t>(k) * n + j];
      }
    }
  }
...
TEST_P(KamalaginARunFuncTestsMatMult, DenseMatMult_Strassen) {
  ExecuteTest(GetParam());
}

const std::array<TestType, 6> kTestParam = {std::make_tuple(0, "n0"),
                                            std::make_tuple(1, "n1"),
                                            std::make_tuple(2, "n2"),
                                            std::make_tuple(3, "n3"),
                                            std::make_tuple(5, "n5"),
...
const auto kTestTasksList =
    std::tuple_cat(ppc::util::AddFuncTask<KamalaginAMatMultStrassenMPI, InType>(kTestParam, PPC_SETTINGS_kamalagin_a_mat_mult_strassen),
                   ppc::util::AddFuncTask<KamalaginAMatMultStrassenSEQ, InType>(kTestParam, PPC_SETTINGS_kamalagin_a_mat_mult_strassen));
