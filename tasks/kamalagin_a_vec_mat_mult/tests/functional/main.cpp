...
TEST_P(KamalaginAVecMatMultTestsProcesses, MatVec) { ExecuteTest(GetParam()); }

const std::array<TestType, 8> kTestParam = {
    std::make_tuple(0, 0, "empty"),
    std::make_tuple(0, 5, "n0"),
    std::make_tuple(5, 0, "m0"),
    std::make_tuple(1, 1, "1x1"),
    std::make_tuple(1, 10, "1x10"),
    std::make_tuple(10, 1, "10x1"),
    std::make_tuple(5, 3, "5x3"),
    std::make_tuple(257, 10, "257x10"),
};

const auto kTestTasksList =
    std::tuple_cat(ppc::util::AddFuncTask<KamalaginAVecMatMultMPI, InType>(
                       kTestParam, PPC_SETTINGS_kamalagin_a_vec_mat_mult),
                   ppc::util::AddFuncTask<KamalaginAVecMatMultSEQ, InType>(
                       kTestParam, PPC_SETTINGS_kamalagin_a_vec_mat_mult));

const auto kGtestValues = ppc::util::ExpandToValues(kTestTasksList);

const auto kTestName =
    KamalaginAVecMatMultTestsProcesses::PrintFuncTestName<KamalaginAVecMatMultTestsProcesses>;
